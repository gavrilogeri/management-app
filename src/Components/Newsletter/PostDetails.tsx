...
export default function PostDetails(props: any) {
    ...
    const deletePost = (postId: number): void => {
        ...
        }).then(() => { setOpenForm(false); alert("POST SUCCESFULLY DELETED") })

    }

    const updatePost = (id: number, title: string, body: string, userId: number): void => {
        ...
        <div className={classes.root}>
            <form autoComplete="off">
                <Grid container>
                    ...
                    <Grid item xs={12}>
                        <Typography className={classes.commentsHeading} variant="h5" gutterBottom>
                            Comments:
                   </Typography>
                    </Grid>
                    {data?.map((item: Comment) =>
                        <Grid item xs={6} key={item.id} className={classes.commentCardStyle} >
                            <CommentCard
